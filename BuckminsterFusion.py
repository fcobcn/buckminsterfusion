...
def run(context):
    ...
    try:
        ...
        cmdDef = ui.commandDefinitions.addButtonDefinition(
            'BuckminsterFusionCmd',
            'Buckminster Fusion',
            'Creates a parametric geodesic sphere (Buckminster dome)',
            'resources'
        )
...
def stop(context):
    try:
        ...
        # Clean up the UI
        cmdDef = ui.commandDefinitions.itemById('BuckminsterFusionCmd')
        if cmdDef:
            cmdDef.deleteMe()

        solidPanel = ui.allToolbarPanels.itemById('SolidCreatePanel')
        cntrl = solidPanel.controls.itemById('BuckminsterFusionCmd')
        if cntrl:
            cntrl.deleteMe()
